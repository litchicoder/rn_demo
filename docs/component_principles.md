...
```mermaid
graph LR
    A(("👨‍💻 你 (代码工程师)")) -- "声明期望状态变灰不可点" --> B{"🧑‍🍳 React 引擎"}
    B -- "计算最优变化路径交给底层去干活" --> C["📱 手机瞬间变成你想要的超生动画面"]
```
...
stateDiagram-v2
    ...
    出生_Mount --> 活着_Update : 首次登场画面出现在屏幕上
    Note right of 出生_Mount : 🚀 useEffect (触发网络等副作用) - 举例：跑去问服务器要首屏数据

    活着_Update --> 活着_Update : 随用户点击或屏幕划动，State无间歇随时更新
    
    活着_Update --> 死亡_Unmount : 用户划走到其他页面，组件彻底从屏幕消失
    Note right of 死亡_Unmount : 🧹 useEffect 内部 return (大扫除) - 举例：注销消息听众、关掉滴答响的倒计时定时器
